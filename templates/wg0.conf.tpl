...
%{~ else}
%{~ if node.os == "linux" }
PostUp   = iptables -A FORWARD -i %i -j ACCEPT; iptables -A FORWARD -o %i -j ACCEPT; iptables -t nat -A POSTROUTING -s ${node.linux.block} -o ${node.linux.interface} -j MASQUERADE${node.linux.up}
PostDown = iptables -D FORWARD -i %i -j ACCEPT; iptables -D FORWARD -o %i -j ACCEPT; iptables -t nat -D POSTROUTING -s ${node.linux.block} -o ${node.linux.interface} -j MASQUERADE${node.linux.down}
%{~ endif }
%{~ endif }
